...
RSpec.describe StatIncrementer do
  ...
  subject { described_class.new(short_code: url.short_url, ip: ip) }

  describe '#let' do
    context 'stat record is not present yet' do
      it 'should create one stat item' do
        subject.call
        count = Stat.where(url_id: url.id, ip: ip).count
    ...
    context 'requests from different ips' do
      it 'should create 2 stat records' do
        subject.call
        described_class.call(short_code: url.short_url, ip: ip_2)
        count = Stat.where(url_id: url.id).count
